import style from "component/global/ItemList/ItemList.module.scss";
const ItemList = ({ title, slug, image, color, header, logo, tag = "h2", variant, category, bigImg, setAttrLinkTo }) => {
    ...
    const content = (
        <>
            <TAG className={style["item-list__title"]}>
                <a href={slug} className={`extend-link`} {...setAttrLinkTo}>
                    {title}
                </a>
            </TAG>
    ...
    return (
        <article className={`${style["item-list"]} ${style[variant]} ${isMediaObject && "d-i-flex align-center"} extend-link--outside`}>
            {isMediaObject && <img loading="lazy" src={image} alt="" width={bigImg ? "73" : "50"} height={bigImg ? "73" : "50"} className={`${style["item-list__image"]} ${bigImg ? style["item-list__bigImage"] : ""}`} />}
